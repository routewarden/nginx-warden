#!/usr/bin/env bash
# ==============================================================================
# RouteWarden Live Testing Script (NGINX / OpenResty Lua)
...
wait_for_nginx() {
    log_info "Waiting for NGINX/OpenResty to be ready on port 8080..."
    local retries=30
    while ! curl -s -f "http://${BASE_HOST}:8080/" > /dev/null 2>&1; do
        sleep 1
        retries=$((retries - 1))
        if [ "$retries" -le 0 ]; then
            echo -e "${RED}[ERROR] NGINX/OpenResty did not become ready in time.${NC}"
            exit 1
        fi
    done
    log_info "NGINX/OpenResty is responding. Starting test suite!"
}
...
test_response_modes() {
    ...
    if [ "$status" = "200" ] && [[ "$body" == *"APP_NAME=Laravel"* ]] && [[ "$body" == *"fake_honey_db_password"* ]]; then
    ...
    set -e
    if [ "$silent_exit_code" -ne 0 ] || [ "$silent_status" = "444" ] || [ "$silent_status" = "403" ] || [ "$silent_status" = "000" ]; then
...
test_header_inspection() {
    ...
    if [ "$status" = "200" ] && [[ "$body" == *"OK: Upstream Reached (8094)"* ]]; then
...
test_security_logging() {
    ...
    if command -v docker > /dev/null 2>&1 && docker compose ps --status running | grep -q "nginx"; then
        local logs
        logs=$(docker compose logs --tail=50 nginx 2>/dev/null || true)
        ...
        # Check for structured NGINX error log
        if echo "$logs" | grep -qi "routewarden_block" && echo "$logs" | grep -qi "path_blocked"; then
            log_pass "Structured NGINX security event logged with reason 'path_blocked'"
        else
            log_fail "NGINX security event" "routewarden_block with reason path_blocked" "$logs"
        fi
    else
        log_info "Docker container not detected; tested request dispatch for security_log."
...
main() {
    echo -e "${CYAN}======================================================================${NC}"
    echo -e "${CYAN}   RouteWarden for NGINX & OpenResty — Live Test & Mode Verification   ${NC}"
    echo -e "${CYAN}======================================================================${NC}"

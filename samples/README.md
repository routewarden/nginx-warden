# RouteWarden Samples & Live Testing Environment (NGINX / OpenResty)

This directory provides a multi-port Docker Compose environment and an automated test suite (`test.sh`) to verify all RouteWarden response modes, security flags, path-evasion protections, and IP filtering capabilities in NGINX / OpenResty.

## Architecture

The test environment provisions:
1. **OpenResty Container**: Runs OpenResty with RouteWarden directly mounted/copied from the local library (`samples/Dockerfile`) and exposes 15 dedicated test ports mapped in `samples/nginx.conf`.
...
```
samples/
├── nginx.conf           # Multi-port OpenResty configuration exercising all modes and flags
├── Dockerfile           # Builds OpenResty image with RouteWarden loaded
├── docker-compose.yml   # Multi-container orchestration (OpenResty + Honeypot)
